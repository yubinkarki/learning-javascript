...
// you can think of closure as a way to “remember”
// and continue to access a function’s scope (its variables) even once the function has finished running
...
// gets a reference to the inner 'add()' function
// with closure over the 'x' parameter of the outer 'makeAdder()' function
...
// the User() function serves as an outer scope
// that holds the variables username and password, as well as the inner doLogin() function
// these are all private inner details of this User module that cannot be accessed from the outside world
// we are not calling "new User()"" here, on purpose
// despite the fact that probably seems more common to most readers
...
// a whole new scope is created, and thus a whole new copy of each of these inner variables/functions
// we assign this instance to fred - if we run User() again
// we’d get a new instance entirely separate from fred
...
// normally, you’d think the inner variables like username and password have gone away
...
// there’s a good chance that with just this brief glimpse at closure and the module pattern
// some of it is still a bit confusing
