...
// another very commonly misunderstood concept in JS is the this keyword
// while it may often seem that this is related to “object-oriented patterns”
// in JS this is a different mechanism
...
// JS will automatically use that object’s internal prototype reference
...
// but because 'awesome' is prototype-linked to 'nice'
// JS automatically falls back to looking for 'a' on the 'nice' object, where it’s found
// this linkage may seem like a strange feature of the language
// the most common way this feature is used and I would argue, abused
...
// where you intentionally design your linked objects
// to be able to delegate from one to the other for parts of the needed behavior

/* OLD AND NEW */

// some of the JS features we’ve already covered
// and certainly many of the features covered in the rest of this series
// are newer additions and will not necessarily be available in older browsers
// in fact, some of the newest features in the specification
// aren’t even implemented in any stable browsers yet
...
// used to refer to taking the definition of a newer feature
// and producing a piece of code that’s equivalent to the behavior
// but is able to run in older JS environments
// for example, ES6 defines a utility called Number.isNaN(..)
// to provide an accurate, non-buggy check for NaN values, deprecating the original isNaN(..) utility
// but it’s easy to polyfill that utility so that you can start using it in your code
// regardless of whether the end user is in an ES6 browser or not
...
// you should be really, really careful in implementing a polyfill yourself
// to make sure you are adhering to the specification as strictly as possible
...
// essentially, your source code is authored in the new syntax form
...
// only to have it transpiled away to older code
...
//   you should prefer writing newer and cleaner syntax
//   not only for yourself but for all other members of the development team
//   if you transpile only for older browsers
//   but serve the new syntax to the newest browsers
//   you get to take advantage of browser performance optimizations with the new syntax
...
//   which provides earlier feedback to the JS committee (TC39)
//   if issues are found early enough
//   they can be changed/fixed before those language design mistakes become permanent
...
// as you can see, it checks to see if the arguments[0] value is void 0 (aka undefined)
// and if so provides the 2 default value; otherwise, it assigns whatever was passed
// in addition to being able to now use the nicer syntax even in older browsers
// looking at the transpiled code actually explains the intended behavior more clearly
