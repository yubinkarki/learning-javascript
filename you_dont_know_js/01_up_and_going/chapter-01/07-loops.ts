...
// in other words, repeating only while the condition holds is the job of programming loops
...
// the only practical difference between these loops is
// whether the conditional is tested before the first iteration or after
// in either form, if the conditional tests as false, the next iteration will not run
// that means, if the condition is initially false
// a while loop will never run, but a do..while loop will run just the first time
...
// also, we can observe that it's awfully easy to create a loop
// that would otherwise run forever without a breaking mechanism
...
// this is not necessarily a practical form you'd want to use for you loops
// it's presented here for illustration purposes only
...
// there are other specialized loop forms that are intended to iterate over specific values
// such as the properties of an object
