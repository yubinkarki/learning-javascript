...
// the built-in types and subtypes we’ve just discussed
// have behaviors exposed as properties and methods that are quite powerful and useful
...
// briefly, there is a String (capital S) object wrapper form
...
// when you use a primitive value like "ok" as an object by referencing a property or method
...
// a number can be wrapped by a Number object and a boolean can be wrapped by a Boolean object
// for the most part, you don't need to worry about or directly use these object wrapper forms of the values
// prefer the primitive value forms in practically all cases and JS will take care of the rest

/* COMPARING VALUES */

// there are two main types of value comparison that you will need to make in your JS programs:
// equality and inequality
// the result of any comparison is a strictly boolean value (true or false)
...
// whereas implicit coercion is when the type conversion can happen
// as more of a non-obvious side effect of some other operation
// you’ve probably heard sentiments like “coercion is evil”
// drawn from the fact that there are clearly places where coercion can produce some surprising results
...
// it's important to remember than a non-boolean value only follows this coercion
// if it's actually coerced to a boolean
// it’s not all that difficult to confuse yourself with a situation
// that seems like it’s coercing a value to a boolean when it’s not
...
// JS notices that the types do not match
// so it goes through an ordered series of steps to coerce one or both values to a different type
...
// in such a simple example, it doesn’t really seem to matter which way that process goes
// as the end result is the same
// there are more complex cases where it matters not just what the end result of the comparison is
// but how you get there
// you should take special note of the == and === comparison rules
// if you’re comparing two non-primitive values, like objects (including function and array)
// because those values are actually held by reference
// both == and === comparisons will simply check whether the references match
// not anything about the underlying values
// for example, arrays are by default coerced to strings
// by simply joining all the values with commas (,) in between
...
// referred to in the specification as “relational comparison”
// typically they will be used with ordinally comparable values like numbers
// it’s easy to understand that 3 < 4
// but JS string values can also be compared for inequality
...
// notably, there are no “strict inequality” operators
// that would disallow coercion the same way === “strict equality” does

var testOne = 41;
var testTwo = "400";
var testThree = "42";

console.log("testOne < testTwo", testOne < testTwo); // true
console.log("testTwo < testThree", testTwo < testThree); // true "4" === "4" >> "0" < "2"
