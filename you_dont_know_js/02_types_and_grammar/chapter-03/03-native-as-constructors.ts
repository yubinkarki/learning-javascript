...
// but the literal form creates the same sort of object as the constructor form does (that is, there is no non-wrapped value)
// just as we’ve seen above with the other natives, these constructor forms should generally be avoided, unless you really know you need them
...
// instead of providing that value as contents of the array, it’s taken as a length to “pre-size the array” (well, sorta)
...
// more than just confusing console output, x and y from the above code snippet actually behave the same in some cases but differently in others:
