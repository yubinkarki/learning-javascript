...
// i say "integer" in quotes because it’s long been a criticism of JS that there’s not true integers, as there are in other languages
...
// the most (in)famous side effect of using binary floating-point numbers (which, remember, is true of all languages that use IEEE 754 — not just JS as many assume/pretend) is:
0.1 + 0.2 === 0.3; // false
...
// this tiny value is often called “machine epsilon” which is commonly 2^-52 (2.220446049250313e-16) for the kind of numbers in JS
...
// because of how numbers are represented, there is a range of “safe” values for the whole number “integers”
...
// the main scenario in which JS programs are confronted with such large numbers is when dealing with 64-bit IDs from databases, etc
// 64-bit numbers cannot be represented accurately with the number type, so they must be stored in (and transmitted to/from) JS using string representation
// numeric operations on such large ID number values (besides comparison, which will be fine with strings) aren’t all that common, thankfully
// but if you do need to perform math on these very large values, for now you’ll need to use a big number utility
