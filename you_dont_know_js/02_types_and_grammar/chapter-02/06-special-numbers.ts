...
// (or values that can be interpreted as regular numbers in base 10 or base 16)
// will result in the operation failing to produce a valid number
// in which case you will get the NaN value
// NaN literally stands for “not a number”
// though this label/description is very poor and misleading, as we’ll see shortly
// it would be much more accurate to think of NaN as being an “invalid number”, “failed number"
...
// NaN is a kind of “sentinel value”
// (an otherwise normal value that’s assigned a special meaning)
// that represents a special kind of error condition within the number set
// the error condition is, in essence
// “I tried to perform a mathematic operation but failed, so here’s the failed number result instead”
// so, if you have a value in some variable
// and want to test to see if it’s this special failed-number NaN
// you might think you could directly compare to NaN itself
// as you can with any other value, like null or undefined - nope
...
// easy enough, right?
// we use the built-in global utility called isNaN(..)
// and it tells us if the value is NaN or not - problem solved!
// not so fast
...
// Developers from traditional compiled languages like C are probably used to seeing
// either a compiler error or runtime exception, like “divide by zero” for an operation like:
...
// as you can see, -Infinity (aka Number.NEGATIVE_INFINITY)
...
// like addition or subtraction, in which case you’d get Infinity or -Infinity
...
// if an operation like addition results in a value that’s too big to represent
// the IEEE 754 “round-to-nearest” mode specifies what the result should be
// so, in a crude sense, Number.MAX_VALUE + Math.pow( 2, 969 )
// is closer to Num ber.MAX_VALUE than to Infinity, so it “rounds down”
...
// it’s almost philosophical to ask:
// “what is infinity divided by infinity?” Our naive brains would likely say “1” or maybe “infinity”
