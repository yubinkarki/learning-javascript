...
// while it may confuse the mathematics-minded reader
// JS has both a normal zero 0 (otherwise known as a positive zero +0) and a negative zero -0
...
// though that was not the common case until fairly recently
// so some older browsers you encounter may still report it as 0
...
// in addition to stringification of negative zero being deceptive to hide its true value
// the comparison operators are also (intentionally) configured to lie:
console.log("a == c", a == c); // true
console.log("-0 == 0", -0 == 0); // true
console.log("-0 > 0", -0 > 0); // false

// clearly, if you want to distinguish a -0 from a 0 in your code
// you can’t just rely on what the developer console outputs
// so you’re going to have to be a bit more clever:
...
// similarly, -0 lies and pretends that it’s equal (even === strict equal) to regular 0
// so you have to use the somewhat hackish isNegZero(..) utility we suggested above

// as of ES6, there’s a new utility that can be used to test two values for absolute equality
// without any of these exceptions
...
// as the operators are likely much more efficient and certainly are more idiomatic/common
