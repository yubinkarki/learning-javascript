...
// while integers can range up to roughly 9 quadrillion safely (53 bits), there are some numeric operations (like the bitwise operators) that are only defined for 32-bit numbers
...
// this works because the | bitwise operator only works for 32-bit integer values (meaning it can only pay attention to 32 bits and any other bits will be lost)
...
// by convention (mostly from C-language programming), to represent the undefined value standalone by using void, you’d use void 0 (though clearly even void true or any other void expression does the same thing)
...
// here, the setTimeout(..) function returns a numeric value (the unique identifier of the timer interval, if you wanted to cancel it)
// but we want to void that out so that the return value of our function doesn’t give a false positive with the if statement
