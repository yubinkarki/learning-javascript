...
// in many other languages, values can either be assigned/passed by value-copy
// or by reference-copy depending on the syntax you use
// for example, in C++ if you want to pass a number variable into a function
// and have that variable’s value updated
// you can declare the function parameter like int& myNum
// and when you pass in a variable like x, myNum will be a reference to x
// references are like a special form of pointers, where you obtain a pointer to another variable (like an alias)
// if you don’t declare a reference parameter
// the value passed in will always be copied, even if it’s a complex object
...
// so if you have 10 different references, they are all always distinct references to a single shared value
...
// instead, the type of the value solely controls whether
// that value will be assigned by value-copy or by reference-copy
...
// in the above snippet, because 2 is a scalar primitive
// 'a' holds one initial copy of that value, and b is assigned another copy of the value
...
// so, when using either reference to modify (.push(4)) the actual shared array value itself
...
// when we make the assignment f = [4, 5, 6]
// we are doing absolutely nothing to affect where e is still referencing ([1,2,3])
...
// this is in no way affecting where the initial reference e is pointing
...
// you need to manually make a copy of it
// so that the reference passed doesn’t still point to the original
// for example:
function bar(y) {
  y.push(4); // [1, 2, 3, 4]
...
// you could just box the value in its Number object wrapper
// it is true - a copy of the reference to this Number object will be passed to the function
// but unfortunately, having a reference to the shared object
// is not going to give you the ability to modify the shared primitive value, like you may expect:
...
// if a Number object holds the scalar primitive value 2
// that exact Number object can never be changed to hold another value
// you can only create a whole new Number object with a different value
// when myNum is used in the expression myNum + 1
// the underlying scalar primitive value 24 is unboxed (extracted) from the Number object automatically
// so the line myNum = myNum + 1 very subtly changes myNum from being a shared reference
// to the Number object, to just holding the scalar primitive value 25
// as a result of the addition operation 24 + 1
// therefore, numTwo on the outside still references
// the original unmodified/immutable Number object holding the value 24

// references are quite powerful, but sometimes they get in your way
// and sometimes you need them where they don’t exist
